...
var loanExtensionList = {
			...
			batchRemove:function(){
				...
				for(var i=0;i<rows.length;i++){
					if(1 != rows[i].requestStatus){
						$.messager.alert("操作提示","只能删除处理状态为 '待客户经理提交' 的展期申请!","error");
						return;
					}
			...
			searchProjectDeil:function(row){
				var url = BASE_PATH+"foreLoanExtensionController/toEditLoanExtensionApply.action?projectId="+row.extensionProjectId+"&newProjectId="+row.projectId+"&biaoZhi=3"
