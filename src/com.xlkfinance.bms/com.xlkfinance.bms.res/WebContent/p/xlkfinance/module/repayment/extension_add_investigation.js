...
var extension = {
		...
		init:function(projectId,newProjectId){
			...
			// 老项目的抵质押物信息是不能修改的
			applyCommon.dyxx_gridOnLoadSuccess(extension.hisProjectIds);
			// 老项目的个人保证信息是不能修改的
			applyCommon.perGuarantee_gridOnLoadSuccess(extension.hisProjectIds);
			// 老项目的企业法人保证信息是不能修改的
			applyCommon.corGuarantee_gridOnLoadSuccess(extension.hisProjectIds);
			// 老项目的质押押物信息是不能修改的
			applyCommon.dyxx_grid_checkOnLoadSuccess (extension.hisProjectIds);
			// 加载展期基本信息
			extension.loadLoanInfo(temporaryProjectId,newProjectId,projectId,loanId);
			//还款周期置灰不让操作 
		...
		saveExtensionInvestigation:function(){
			...
			// 获取授信额度和贷款金额
			var loanAmt = $("#investigationForm input[name='extensionAmt']").val();
			...
			// 判断展期结束日期是否超过授信日期
			var creditDate = $("#credtiEndDt").datebox('getValue');
			var extensionDate = $("#planRepayLoanDt").datebox('getValue');
			if(extensionDate > creditDate){
				if(!confirm('展期结束日期超过授信期限，请确认!')){
					return ;
				}
			}
			...
			$("#investigationForm input[name='loanId']").val(loanId);
	 		...
	 		if($('#repayFun').combobox('getText') == "其他还款方式"){
	 			$.messager.confirm("操作提示","在其它还款方式下，会清空原先录入的还款计划表数据，请确认是否继续保存？",
					function(r) {
						if(r){
		...
		getLoanExtension:function(projectId){
			...
			$.ajax({
		        ...
		        async:false, 
		    	url : BASE_PATH+'loanExtensionController/getProjectExtension.action',
				dataType: "json",
		...
		loadLoanInfo:function(temporaryProjectId,newProjectId,projectId,loanId){
			...
			$.post(url,{projectId : temporaryProjectId},
			  	function(data){
					...
					if(loanId!=-1){
						//加载展期选择 展期金额
						var loanExtension = extension.getLoanExtension(temporaryProjectId);
						data.extensionNum = loanExtension.selectCycleNum;
				    	data.extensionAmt = loanExtension.extensionAmt;
					}
					// 加载授信信息
					var hisProjectIds = extension.hisProjectIds.split(",");
					applyCommon.loadCreditsInfo(hisProjectIds[hisProjectIds.length-1]);
		...
		accessControl:function(projectName,workflowKey){
			...
			if("task_ApprovalOfficerReview" == workflowKey || "task_OfflineMeetingResult" == workflowKey){
				$("#projectName").attr('readOnly','readOnly');
			}else if ("task_ExtensionRequest" == workflowKey){ // 申请中的申请
