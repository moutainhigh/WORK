...
<script type="text/javascript">
	...
	//项目名称format
	 ...
	 function toTransitMonitorShow() {
			...
			if (row.length == 1) {
				url = "${basePath}foreAfterLoanController/toTransitMonitorShow.action?projectId="+row[0].projectId;
				parent.openNewTab(url, "查看在途业务监控信息", true);
	...
	function subCheckDocumentForm() {
		...
		var checkHours = $("#checkDocumentCheckHours").combobox("getValue");
		checkHours+=":00:00";
...
/* 			$.ajax({
	...
	$(document).ready(function() {
		setCapitalOrgInfo2("makeLoansRes",'');
		initUploadify();
		$('#house_list_grid').datagrid({
			//当物业列表数据加载完毕
	...
	function cxportTransitMonitor() {
		$.ajax({url : BASE_PATH
			+ 'templateFileController/checkFileUrl.action',
			data:{templateName : 'ZTYWJK'},
			dataType:'json',
			success : function(data) {
				if (data == 1) {
					var projectName =$("#projectName").textbox('getValue');
					var businessSourceStr =$("#businessSourceStr").textbox('getValue');
					var foreAfterMonitorStatus =$("#foreAfterMonitorStatus").combobox('getValue');
					var planRepaymentDate =$("#planRepaymentDate").textbox('getValue');
					var planRepaymentEndDate =$("#planRepaymentEndDate").textbox('getValue');
					var makeLoansRes =$("#makeLoansRes").combobox('getValue');
					window.location.href ="${basePath}foreAfterLoanController/exportTransitMonitorIndexList.action?businessSourceStr="+businessSourceStr+"&foreAfterMonitorStatus="+foreAfterMonitorStatus+"&planRepaymentDate="+planRepaymentDate+"&planRepaymentEndDate="+planRepaymentEndDate+"&makeLoansRes="+makeLoansRes+"&projectName="+projectName+"&page=-1";
				} else {
					alert('在途业务监控导出模板不存在....，请上传模板后重试');
				}
			}
		}) 
	}
...
<body class="easyui-layout">
	...
	<div id="toolbar" class="easyui-panel" style="border-bottom: 0;">
		<form action="<%=basePath%>foreAfterLoanController/transitMonitorIndexList.action" method="post" id="searchFrom" name="searchFrom" >
			<!-- 查询条件 -->
			<div style="padding:5px">
				<table>
					<tr>
						...
						<td width="85" align="right" height="28">监控状态：</td>
						<td>
							<select class="select_style easyui-combobox" width="129px;" editable="false" name="foreAfterMonitorStatus" id="foreAfterMonitorStatus" style="width: 95px">
					...
					<td width="90" align="right"  height="28">预计回款日期：</td>
						...
						<td width="80" align="right"  height="28">资金来源：</td>
						<td>
							<input name="makeLoansRes" id="makeLoansRes" editable="false"  style="width: 80px;" class="easyui-combobox"/>
						</td>
					</tr>
					<tr>
						<td colspan="3">
		...
		<div style="padding-bottom:5px">
			<shiro:hasAnyRoles name="CHECK_LITIGATION,ALL_BUSINESS">
		     ...
		     <shiro:hasAnyRoles name="TO_TRANSIT_MONITOR_SHOW,ALL_BUSINESS">
		      <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-view" plain="true" onclick="toTransitMonitorShow()">查看</a>
		     </shiro:hasAnyRoles>
		</div>
		<div style="padding-bottom: 5px">
    <shiro:hasAnyRoles name="EXPORT_TRANSIT_MONITOR,ALL_BUSINESS">
     <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-export" plain="true" onclick="cxportTransitMonitor()">导出</a>
     </shiro:hasAnyRoles>
    </div>
	</div>
	<div class="listDiv" style="height:100%;">
	<table id="grid" title="在途业务监控列表"  class="easyui-datagrid"
		style="height:100%;width: auto;"
		data-options="
		    url: '<%=basePath%>foreAfterLoanController/transitMonitorIndexList.action',
		    method: 'POST',
		    rownumbers: true,
		    singleSelect: true,
		    pagination: true,
		...
		<thead><tr>
            ...
            <th data-options="field:'foreAfterMonitorStatus',sortable:true,formatter:formatterForeAfterMonitorStatus" align="center" halign="center">监控状态</th>
   ...
   <form id="checkDocumentForm" name="checkDocumentForm" action="${basePath}integratedDeptController/saveCheckDocumentHis.action" method="post">
     ...
     <table style="width: 100%; height: 270px;" id="checkDocumentForm_table">
       ...
       <tr>
        ...
        <select class="easyui-combobox" name="checkHours" id="checkDocumentCheckHours" style="width: 50px;">
      			  <option value="00">00</option>
	              <option value="01">01</option>
	              <option value="02">02</option>
	              <option value="03">03</option>
       ...
       <tr>
        ...
        <td><input name="checkStatus" id="checkDocumentCheckStatus" style="width: 100px;" data-options="validType:'selrequired',required:true" class="easyui-combobox" editable="false"/></td>
       </tr>
       <tr>
    ...
    <input type="hidden" id="checkDocumentId" name="pid"/>
     ...
     <form id="checkDocumentAutoForm" name="checkDocumentAutoForm" action="" method="post" style="display: none;">
		...
		<table style="width:55%;padding: 10px;float: left; " id="check_doc_table" >
			<tr>
