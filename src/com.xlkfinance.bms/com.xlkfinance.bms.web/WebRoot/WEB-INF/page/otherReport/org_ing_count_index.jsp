		...
		$.ajax({url : BASE_PATH
					...
					success : function(data) {
						if (data == 1) {
							...
							window.location.href ="${basePath}otherReportController/orgIngCountExportList.action?chooseMonthOrWeek="+mothOrWekNum+"&reMonth="+reMonth+
							"&pmUserName="+pmUserName+"&projectNumber="+projectNumber+"&projectName="+projectName+"&orgOrgName="+orgName+"&includeWt="+includeWt+"&page=-1";
						} else {
							alert('合作机构结清详情的导出模板不存在....，请上传模板后重试');
						}
					}
				})
	}
 ...
 <div data-options="region:'center',border:false">
  <div id="scroll-bar-div">
   ...
   <div id="toolbar" class="easyui-panel" style="border-bottom: 0;">
    <form action="<%=basePath%>otherReportController/orgIngCountList.action" method="post" id="searchFrom" name="searchFrom">
     <!-- 查询条件 -->
   ...
   <div class="dksqListDiv" style="height: 100%;">
    <table id="grid" title="赎楼贷合作机构在途详情列表" class="easyui-datagrid" style="height: 100%; width: auto;"
     data-options="
		    url: '<%=basePath%>otherReportController/orgIngCountList.action?mothOrWekNum='+${mothOrWekNum}+'&reMonth='+reMonth+'&orgOrgName='+orgName+'&includeWt='+includeWt,
		    method: 'POST',
		    rownumbers: true,
		    singleSelect: false,
		    pagination: true,
