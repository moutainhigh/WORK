 		...
 		if(row[0].status != 3){
 	 		var url ="";
 	   		if (row[0].backFeeApplyHandleStatus>=<%=com.xlkfinance.bms.common.constant.Constants.APPLY_REFUND_FEE_STATUS_3%>) {
 	   		    url = "${basePath}refundFeeController/edit.action?projectId="+row[0].projectId+"&pid="+row[0].pid+"&type=3&param='refId':'"+row[0].projectId+"','projectName':'"+row[0].projectName+"','projectId':'"+row[0].projectId+"'";
 			}else{
 	   		    url = "${basePath}refundFeeController/edit.action?projectId="+row[0].projectId+"&pid="+row[0].pid+"&type=3";
 			}
 			parent.openNewTab(url,"退尾款办理",true);
 		}else{
 			$.messager.alert("操作提示","退尾款办理只有申请中和已收费才能进行编辑！","error");
 		}
...
function print(){
	var row = $('#grid').datagrid('getSelections');
 	if (row.length == 1) {
 		url = "${basePath}refundFeeController/print.action?pid="+row[0].pid+"&projectId="+row[0].projectId+"&type=3&processDefinitionKey=<%=com.xlkfinance.bms.common.constant.Constants.REFUND_TAIL_MONEY_WORKFLOW_ID%>";
			parent.openNewTab(url, "退尾款打印", true);
		} else if (row.length > 1) {
			$.messager.alert("操作提示", "每次只能修改一条数据,请重新选择！", "error");
		} else {
			$.messager.alert("操作提示", "请选择数据", "error");
 ...
 <div data-options="region:'center',border:false">
  <div id="scroll-bar-div">
   ...
   <div id="toolbar" class="easyui-panel" style="border-bottom: 0;">
    <form action="<%=basePath%>refundFeeController/list.action?type=3" method="post" id="searchFrom" name="searchFrom">
     <!-- 查询条件 -->
   ...
   <div class="dksqListDiv" style="height: 100%;">
    <table id="grid" title="退尾款申请列表" class="easyui-datagrid" style="height: 100%; width: auto;"
     data-options="
		    url: '<%=basePath%>refundFeeController/list.action?type=3',
		    method: 'POST',
		    rownumbers: true,
		    singleSelect:true,
		    pagination: true,
   ...
   <div id="refund_fee_confirm_dialog" class="easyui-dialog" buttons="#submitDiv1" style="width: 430px; height: 210px; padding: 10px;" closed="true">
    <form id="refundFeeConfirmForm" name="refundFeeConfirmForm" action="${basePath}refundFeeController/refundFeeConfirm.action" method="post">
     ...
     <input type="hidden" name="type" value="3"> <input type="hidden" name="projectId" id="refundFeeConfirmProjectId">
    </form>
   </div>
   <div id="submitDiv1">
