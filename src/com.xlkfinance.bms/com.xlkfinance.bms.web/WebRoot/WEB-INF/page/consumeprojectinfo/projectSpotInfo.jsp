				...
				<c:if test="${projectSpotInfoList != null && fn:length(projectSpotInfoList) > 0}">
					<c:forEach items="${projectSpotInfoList}" var="item" varStatus="status">
						<div style="padding: 10px 0 0 0;">
							<div class="easyui-panel" title="外访视察报告--产权证编号--${item.housePropertyCard}" data-options="collapsible:true">
...
<%-- 													</c:if> --%>
													<input <c:if test="${foreclosureStatus != 3}">readonly="readonly"</c:if> name="spotInfos[${status.index}].shouldSpotTime" class="easyui-datebox" data-options="required:true,editable:false" value="${item.bizSpotInfo.shouldSpotTime}"/>
									...
									<table style="width: 100%;padding-bottom: 10px;">
										<tbody>
											...
											<tr>
												<td class="label_right">承租人姓名：</td>
												<td >
													<input name="spotInfos[${status.index}].lessee" <c:if test="${foreclosureStatus != 3}">readonly="readonly"</c:if> class="easyui-textbox" value="${item.bizSpotInfo.lessee}" data-options="validType:'length[1,20]'" style=""/>
												</td>
												<td class="label_right1">联系电话：</td>
												<td  colspan="5">
													<input name="spotInfos[${status.index}].leaseUsePhone" <c:if test="${foreclosureStatus != 3}">readonly="readonly"</c:if> class="easyui-textbox" value="${item.bizSpotInfo.leaseUsePhone}" data-options="validType:'telephone'" style=""/>
												</td>
											</tr>
											<tr>
											    <td class="label_right required">已租价格：</td>
												<td>
													<input name="spotInfos[${status.index}].rentPrice" value="${item.bizSpotInfo.rentPrice}" class="easyui-numberbox" data-options="required:true,min:0,max:9999999999,precision:2,groupSeparator:','"/>
													元
												</td>
												 <td class="label_right1 required">租金评估价（月）：</td>
												<td>
													<input name="spotInfos[${status.index}].rentEvaluationPrice" value="${item.bizSpotInfo.rentEvaluationPrice}" class="easyui-numberbox" data-options="required:true,min:0,max:9999999999,precision:2,groupSeparator:','"/>
													元
												</td>
												<td class="label_right required">评估价格来源：</td>
												
												<td colspan="3" style="width:290px">
													<input id="spotInfo_form_evaluationSource_${status.index}" 
													 <c:if test="${foreclosureStatus != 3}">readonly="readonly"</c:if>
													 name="spotInfos[${status.index}].evaluationSource" class="easyui-combobox"
													 editable="false" panelHeight="auto" value="${item.bizSpotInfo.evaluationSource}"/>
													 
													<span class="evaluationSourceDetail">
														<input id="spotInfo_form_evaluationSourceDetail_${status.index}" style="width: 120px;"
														 <c:if test="${foreclosureStatus != 3}">readonly="readonly"</c:if>
														 name="spotInfos[${status.index}].evaluationSourceDetail" class="easyui-combobox"
														 panelHeight="auto" panelHeight="auto" value="${item.bizSpotInfo.evaluationSourceDetail}"/>
													</span>
												</td>
											</tr>
											...
											<tr>
												<td class="label_right">空置天数：</td>
												<td >
													<input name="spotInfos[${status.index}].vacantDays" <c:if test="${foreclosureStatus != 3}">readonly="readonly"</c:if> class="easyui-numberbox" value="${item.bizSpotInfo.vacantDays}" data-options="min:0,max:99999," style=""/>
												</td>
												<td class="label_right1">空置率：</td>
												<td  colspan="5">
												    <c:if test="${empty item.bizSpotInfo.vacantRate or item.bizSpotInfo.vacantRate==0}">
												       	<input name="spotInfos[${status.index}].vacantRate"  readonly="readonly" class="easyui-textbox lessee" value="${vacantRate}" data-options="validType:'length[1,10]'" style=""/>%
												    </c:if>
												     <c:if test="${not empty item.bizSpotInfo.vacantRate and item.bizSpotInfo.vacantRate!=0}">
												       	<input name="spotInfos[${status.index}].vacantRate"  readonly="readonly" class="easyui-textbox lessee" value="${item.bizSpotInfo.vacantRate}" data-options="validType:'length[1,10]'" style=""/>%
												    </c:if>
												</td>
											</tr>
											<tr>
												...
												<td colspan="7">
													<input name="spotInfos[${status.index}].othenRemark" <c:if test="${foreclosureStatus != 3}">readonly="readonly"</c:if> value="${item.bizSpotInfo.othenRemark}" class="easyui-textbox" data-options="multiline:true,validType:'length[1,500]'" style="width: 465px;height:50px;"/>
												</td>
											</tr>
										</tbody>
									</table>
...
$(function(){
	...
	//评估价格来源详情
	function evaluationSourceDetail(_this,data){
		var selectShow = true; 
		var $evaluationDetail = $(_this).siblings("span.evaluationSourceDetail").find("input:first");
		$evaluationDetail.next("span").detach();
		var this_val = $evaluationDetail.val();
			$evaluationDetail.combobox({
				validType:'selrequired',
				valueField:'lookupVal',
				textField:'lookupDesc',
				editable:false,
				url:'${basePath}sysLookupController/getSysLookupValByLookType.action?lookupType='+data.lookupVal,
			});
			
	}
	
	//评估价格来源
	$("input[id*=spotInfo_form_evaluationSource_]").each(function(){
		var _this = this;
		$(this).combobox({
			validType:'selrequired',
			valueField:'lookupVal',
			textField:'lookupDesc',
			url:'${basePath}sysLookupController/getSysLookupValByLookType.action?lookupType=EVALUATION_PRICE_SOURCE',
			onSelect:function(data){
				evaluationSourceDetail(_this,data);
				$(_this).siblings("div.evaluationDetail").find("input:first").combobox("setValue","");
			},
			onLoadSuccess:function(data){
				var val = {lookupVal:$(_this).val()};
				evaluationSourceDetail(_this,val);
			}
		});
	});
	...
	$('#spotInfoForm').form('submit', {
		onSubmit : function() {
			if( $(this).form('validate')){
				...
				var fileLength='${fn:length(projectSpotInfoList)}';
				
				var hasFile=true;
				for(var i=0;i<fileLength;i++){
					var fileTabId="#grid_data_file_"+i;
					var estateRows = $(fileTabId).datagrid('getRows');
					if(estateRows.length < 1){
						//下户图片为空
						$.messager.alert("操作提示","下户图片不能为空!","error");
						hasFile= false;
						$(fileTabId).focus();
					}
				}
