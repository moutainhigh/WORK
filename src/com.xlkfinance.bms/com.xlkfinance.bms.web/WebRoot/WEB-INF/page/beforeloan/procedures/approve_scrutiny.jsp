<%@ page language="java" pageEncoding="UTF-8" contentType="text/html; charset=UTF-8"%>
	...
	<div  class="easyui-panel approval_disappr" data-options="collapsible:true" style="padding:10px" title="审批官审批信息" style="width:auto">
		<!--图标按钮 -->
...
<script type="text/javascript">
	...
	$(function(){
		// 加载数据的路径
		var urlLS = "<%=basePath%>secondBeforeLoanController/getProjectApprovalLs.action?projectId="+projectId;
		...
		applyCommon.loadApproveScrutiny(projectId);
		...
		if("task_ApprovalOfficerReview"==workflowTaskDefKey || "task_OfflineMeetingResult"==workflowTaskDefKey){
			...
			$('.approval_disappr .easyui-combobox').attr('disabled','true');
			$('.approval_disappr input[type="checkbox"]').attr('disabled','disabled');
			$('.approval_disappr .easyui-textbox').attr('disabled','true');
			$('.approval_disappr input:not(.toReady)').attr('readOnly','readOnly');
			$('#include_approve_scrutiny .easyui-linkbutton:not(.toReady)').removeAttr('onclick');
			$('#include_approve_scrutiny .easyui-linkbutton:not(.toReady),.approval_disappr input ,.approval_disappr textarea').attr('disabled','true');
			$('#include_approve_scrutiny .easyui-linkbutton:not(.toReady),.approval_disappr input ,.approval_disappr textarea').attr('readOnly','readOnly');
			$('#include_approve_scrutiny .easyui-linkbutton:not(.toReady),.approval_disappr input ,.approval_disappr textarea').addClass('l-btn-disabled');
		}else{
			$('.approval_disappr .easyui-combobox').attr('disabled','true');
			$('.approval_disappr input[type="checkbox"]').attr('disabled','disabled');
			$('.approval_disappr .easyui-textbox').attr('disabled','true');
			$('.approval_disappr input').attr('readOnly','readOnly');
			$('#include_approve_scrutiny .easyui-linkbutton').removeAttr('onclick');
			$('#include_approve_scrutiny .easyui-linkbutton ,.approval_disappr input,.approval_disappr textarea').attr('disabled','true');
			$('#include_approve_scrutiny .easyui-linkbutton ,.approval_disappr input,.approval_disappr textarea').attr('readOnly','readOnly');
			$('#include_approve_scrutiny .easyui-linkbutton ,.approval_disappr input,.approval_disappr textarea').addClass('l-btn-disabled');
	...
	function openItemAddSpxx(infoType){
		...
		$("#fm_spgsc input[name='projectId']").val(projectId);
	...
	function spgscSave(){
		// 赋值
		$("#approveForm input[name='pid']").val(projectId);
