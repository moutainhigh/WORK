<%@page language="java" pageEncoding="UTF-8"
	contentType="text/html; charset=UTF-8"%>
 	...
 	<%-- 合同 --%>
 	<div  class="easyui-panel control_disappr_contract_make" title="贷款合同（所有贷款合同相关的列表）" data-options="collapsible:true" style="padding:10px" style="width:auto">
			...
			<table id="grid_schtbh" class="easyui-datagrid" 
				...
				data-options="
					...
					<thead><tr>
						...
						<th data-options="field:'contractTypeText',hidden:true"></th>
...
$(document).ready(function(){
	$("#grid_schtbh").datagrid({url:"<%=basePath%>contractInfoController/getAllContractListByProjectId.action?projectId="+projectId });

	$('#grid_schtbh').datagrid({
		onLoadSuccess:function(){
			if("task_ContractSign"==workflowTaskDefKey){
			}else{
				$('.control_disappr_contract_make .easyui-combobox').attr('disabled','disabled');
				$('.control_disappr_contract_make input[type="checkbox"]').attr('disabled','disabled');
				$('.control_disappr_contract_make .easyui-textbox').attr('disabled','disabled');
				$('.control_disappr_contract_make .easyui-linkbutton ,.control_disappr_contract_make a:not(.download)').removeAttr('onclick');
				$('.control_disappr_contract_make a:not(.download) font').attr('color','gray');
				$('.control_disappr_contract_make .easyui-linkbutton ,.control_disappr_contract_make  input,.control_disappr_contract_make  textarea').attr('disabled','disabled');
				$('.control_disappr_contract_make .easyui-linkbutton ,.control_disappr_contract_make  input,.control_disappr_contract_make  textarea').attr('readOnly','readOnly');
				$('.control_disappr_contract_make .easyui-linkbutton ,.control_disappr_contract_make  input,.control_disappr_contract_make  textarea').addClass('l-btn-disabled');
			}
		}
	})
...
function contractMakeFilter(value,row,index){
	...
	var downloadDom = "<a class='download' href='"+BASE_PATH+"beforeLoanController/downloadData.action?path="+row.contractUrl+"&fileName="+row.contractName+"'><font color='blue'>下载</font></a> | ";
