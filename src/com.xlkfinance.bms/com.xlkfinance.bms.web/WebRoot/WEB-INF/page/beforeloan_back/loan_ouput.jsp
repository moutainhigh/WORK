...
<div class="easyui-panel output_dis" title="财务放款" data-options="collapsible:true">
	...
	<form 
		...
		<input type="hidden" name="loanId" id="loanId" value="10007" /> <input
			type="hidden" name="ftBankAcctId" id="ftBankAcctId" value="10007" />
			...
			<div style="width: 500px; text-align: right;">
				<p>
					<a onclick="beforLoanbutton()" data-options="iconCls:'icon-save'"
						class="easyui-linkbutton" id="save-id">保存</a>
					<a href="#" class="easyui-linkbutton" style="" iconCls="icon-calculation" onclick="loancalcInfos()">贷款试算</a>
				</p>
			</div>
		</div>
	</form>
	...
	repayCycleDate=$('#repayCycleDateInfo').textbox('getValue');
	 if(row.length==0&&ftDate!=shouldDate){
				$.messager.confirm('友情提示','实际放款日与应放款日期不相等是否更新还款计划表！',function(r){
					...
					if (r){
						var url = "${basePath}beforeLoanController/getProjectCredtiEndDt.action?projectId="+projectId;
										 	 ...
										 	 $.post("${basePath}beforeLoanController/insertLoadOutputinfo.action",{
												...
												//  $("#ftDate").datebox('setValue',""); 
												 ...
												 var  ftDt=	$("#ftDate").datebox('getValue');
													...
													operRepaymentList("${basePath}beforeLoanController/updateRepaymentPlan.action?pId="+projectId+"&planRepayLoanDt="+repayCycleDate+"&loanId="+loanId+"&planOutLoanDt="+ftDt+"&creditEndDate="+repayCycleDate); 
								     	...
								     	$.post("${basePath}beforeLoanController/insertLoadOutputinfo.action",{
											...
											//  $("#ftDate").datebox('setValue',""); 
											 ...
											 var  ftDt=	$("#ftDate").datebox('getValue');
												...
												operRepaymentList("${basePath}beforeLoanController/updateRepaymentPlan.action?pId="+projectId+"&planRepayLoanDt="+repayCycleDate+"&loanId="+loanId+"&planOutLoanDt="+ftDt+"&creditEndDate="+"");
												$.messager.alert('操作提示', "保存成功！", 'info');
												$("#ftAmt").numberbox('setValue', 0);
												$("#difAmt").numberbox('setValue', 0);
												var  v=$("#ftDate").datebox('getValue');
...
/* 			var row = $("#loanOutputGrid").datagrid('getRows');//获取放款列表
		...
		$('#loanOutputGrid').datagrid({
			url:'${basePath}beforeLoanController/loadOutputinfoList.action?projectId='+projectId
		})
