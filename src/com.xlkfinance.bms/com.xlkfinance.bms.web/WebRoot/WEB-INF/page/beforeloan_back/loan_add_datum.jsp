...
<%@ include file="/config.jsp"%>
	<div class="datum" style="padding:30px 10px 0 10px ">
		<div id="toolbar_data">
			...
			<form id="addOrupdate2" name="addOrupdate2" method="post" enctype="multipart/form-data">
				<table>
					...
					<tr>
						<td align="right" width="120" height="28"><font color="red">*</font>文件路径：</td>
						<td>
							<input class="easyui-filebox" class="download" data-options="buttonText:'选择文件',missingMessage:'选择一个文件'" id="file3" name="file2" style="width:240px;" required="true"/>
						</td>
					</tr>
				</table>
			</form>
	...
	function caozuofiter(value,row,index){
		...
		var dataEditDom = "<a href='javascript:void(0)' class='download' onclick='editData("+row.dataId+",\""+row.fileDesc+"\",\""+row.filePropertyName+"\","+row.fileProperty+",\""+row.fileName+"\")'><font color='blue'>编辑</font></a> | ";
		return downloadDom+dataEditDom+dataDelDom;
	}
	...
	function lookfor(){
		if(projectName){		
			$('.panel-body .easyui-linkbutton ,.panel-body input:not(.download,[type="file"],[type="text"],[type="hidden"])').attr('disabled','disabled');
			$('.panel-body .easyui-linkbutton ,.panel-body input:not(.download,[type="file"],[type="text"],[type="hidden"])').attr('readOnly','readOnly');
			$('.panel-body .easyui-linkbutton ,.panel-body input:not(.download,[type="file"],[type="text"],[type="hidden"]) ').addClass('l-btn-disabled');
			$('.panel-body .easyui-linkbutton ,.panel-body a:not(.download)').removeAttr('onclick');
			$('.panel-body a:not(.download) font').attr('color','gray');
	 	}
		if("task_LoanRequest"==workflowTaskDefKey || "task_ApprovalOfficerReview"==workflowTaskDefKey ||"task_OfflineMeetingResult"==workflowTaskDefKey){
