...
/* 	function submitOfflineMeetingForm(){
		$("#offlineMeeting_projectId").val(newProjectId);
		if($("#meetingResult").text()==""){
			$.messager.alert('操作','会议结果不能为空!','error');
			return false;
		}
		$("#project_Id").val(newProjectId);
		if($("#offlineMeetingFile").textbox('getValue')!=''){
			$('#offlineMeetingForm').attr('enctype','multipart/form-data');
			$('#offlineMeetingForm').form('submit',{
				...
				success:function(data) {
						...
						$("#offlineMeetingFile").textbox('setValue','');
						initBizMeetingInfo();
				},
...
function submitFileForm(){
	...
	if($("#offlineMeetingFiles").textbox('getValue')!=''){
		...
		$('#editfileUploadFormedit').form('submit',{
			/* url:'${basePath}beforeLoanController/obtainOfflineMeeting.action', */
			url:'${basePath}beforeLoanController/saveFileInfo.action?meetingId='+$("#meetingId").val()+"&projectId="+newProjectId,
			success:function(data) {
					if(data!="saveSucc"){
						$.messager.alert('操作','保存失败!','error');
					}else{
	...
	function initBizMeetingInfo(curUserPidss){
		$.ajax({				
			type: "POST",			
	        url: BASE_PATH+"beforeLoanController/initOfflineMeeting.action?projectId="+newProjectId,					
			success : function(data) {	
	...
	<div style="padding:10px 60px 20px 60px">
		<form method="post" id="offlineMeetingForm">
			...
			<table cellpadding="5" style="height: auto;width: 100%">
				...
				<!-- <tr>
					<td align="right" height="28"><span class="cus_red">*</span>会议纪要：</td>
					<td align="left" id="offlineMeetingId">
						<input class="easyui-filebox offlineMeetingInput"  name="offlineMeetingFile" id="offlineMeetingFile" style="width:425px">
					</td>
					<td></td>
					<td></td>
				</tr> -->
		...
		<div id="ws" class="easyui-window" title="人员选择---" data-options="modal:true,closed:true,iconCls:'icon-save',minimizable:false" style="width:750px;height:350px;padding:10px;">
