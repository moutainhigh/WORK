 ...
 <div class="contract_dis">
	<div  style="padding:0px;">
			<div id="contractList">
				 <div id="toolbar_contract"  class="easyui-panel" >
					<!-- 操作按钮 -->
					<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true"  onclick="openCreditsWindow()">新增</a>
					<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editCreditContract()">编辑</a>
					<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" id="generatedContractNumber" onclick="generatedContractNumber()">生成合同编号</a>
					<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="deleteCreditContract()">删除</a>
					<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-reload" plain="true" onclick="publicRefresh('#grid_contract')">刷新</a>
				</div>
				<div class="contractListDiv" id="creditContractDiv">
					<table id="grid_contract" title="利率变更合同" class="easyui-datagrid" 
						style="height:250px;width: auto;"
						data-options="
						    url: '${basePath}creditsContractController/getContractListUrl.action?projectId=${projectId}&contractCatelogKey=LLBG&oldProjectId=-1&refId='+$('#updateinterestChgId').val(),
						    method: 'POST',
						    collapsible:true,
						    rownumbers: true,
						    singleSelect: false,
	...
	<div id="openContract" class="easyui-dialog" style="width:870px;height:400px;padding:10px;" 
		...
		<div class="fitem">
            ...
            <input name="" id="templateName" class="easyui-combobox" style="width: 320px;" editable="false" />
...
<script type="text/javascript">
	...
	// 选择的附件ID
	var assBaseIds="";
	var contractCatelog="LLBG";
	var mortgagorType=1;
	var mgrid="#grid_contract";
	...
	function generatedContractNumber(){
		var rows = $('#grid_contract').datagrid('getSelections');
		// 所选取的合同编号
		var contractIds = "";
		// 判断选取的数量
	 	if ( rows.length == 0 ) {
			...
			for(var i = 0 ; i < rows.length ; i++){
				...
				// 判断是否是临时合同,临时合同全是数字,正式合同包含字母
				if(contractNo.match(reg)){
					// 判断插入符的位置,拼接合同信息ID
					if( i == 0 ){
		...
		$.post("<%=basePath%>contractInfoController/genContractNumber.action",{contractIds : contractIds}, 
			function(ret) {
				...
				if(ret && ret.header["success"] ){
					...
					// 重新加载抵质押物列表信息
					$("#grid_contract").datagrid('load');
					// 清空所选择的行数据
					clearSelectRows("#grid_contract");
				}else{
	...
	function openCreditContract(){
		...
		//绑定合同模板-父合同下拉框
		getContractTemplateCombox('templateName','add_parent_contract_id','templateDialog',1,"LLBG","${projectId}");
	...
	function editCreditContract(){
		...
		if (row.length == 1) {
			...
			$("#edit_templateName").combobox({valueField:'pid',textField:'comboboxTemplateText',url:'${basePath}beforeLoanController/getTempLateList.action?templateCatelogText=LLBG'});
	...
	function ajaxContract(){
		...
		var contractType ="LLBG";		
		var suretyId = $("#suretyId").val()==""?-1:$("#suretyId").val();
		var mortgagorId = $("#mortgagorId").val()==""?-1:$("#mortgagorId").val();	
		loadProjectContract("templateDialog",contractType,tempLateId,"${projectId}","${projectId}","${projectId}",-1,-1,-1,mortgagorType,commonDebtorPids,assBaseIds,interestChgId);
	...
	function saveCreditContract(){
		...
		$.ajax({
	        ...
	        success: function(data){
        		 ...
        		 $(mgrid).datagrid({url: '${basePath}creditsContractController/getContractListUrl.action?projectId=${projectId}&contractCatelogKey=LLBG&oldProjectId=-1&refId='+interestChgId});
        		 $(mgrid).datagrid('reload');
	...
	$(document).ready(function(){ 
		if("task_InterestChangeRequest"==workflowTaskDefKey || "task_ApprovalOfficerReview"==workflowTaskDefKey || "task_ContractSign"==workflowTaskDefKey){
			if("task_ContractSign" != workflowTaskDefKey){
				// 如果不是法务节点,禁用  '生成合同编号' 
				$("#generatedContractNumber").linkbutton("disable");
			}
		}else{
