...
<script type="text/javascript">
	...
	function submitProjectFileArchiveInfoForm(){
		...
		var url="";
		if(!isArchiveEdit){
			url="${basePath}beforeLoanController/saveBizProjectArchive.action";
		}else{
			url="${basePath}beforeLoanController/updateBizProjectArchive.action";
		}
	...
	function uploadArchiveFileInfo(){
		...
		if(isArchiveFileEdit){
			url="${basePath}beforeLoanController/updateBizProjectArchiveFile.action";
		}else{
			url="${basePath}beforeLoanController/saveBizProjectArchiveFile.action";
		}
		...
		projectFileArchive();
		 ...
		 $('#allArchiveFileInfoTable').datagrid({
				onLoadSuccess : function(data){
					if("task_ExtensionRequest"!= workflowTaskDefKey && "task_ProjectArchive"!= workflowTaskDefKey){
						$('.projectFileArchivePaper .easyui-combobox').attr('disabled','disabled');
						$('.projectFileArchivePaper input[type="checkbox"]').attr('disabled','disabled');
						$('.projectFileArchivePaper .easyui-textbox').attr('disabled','disabled');
						$('.projectFileArchivePaper .easyui-linkbutton ,.projectFileArchivePaper a:not(.download)').removeAttr('onclick');
						$('.projectFileArchivePaper a:not(.download) font').attr('color','gray');
						$('.projectFileArchivePaper a:not(.download) span').css('color','gray');
						$('.projectFileArchivePaper .easyui-linkbutton').addClass('');
						$('.projectFileArchivePaper .easyui-linkbutton ,.projectFileArchivePaper  input,.projectFileArchivePaper  textarea').attr('disabled','disabled');
						$('.projectFileArchivePaper .easyui-linkbutton ,.projectFileArchivePaper  input,.projectFileArchivePaper  textarea').attr('readOnly','readOnly');
						$('.projectFileArchivePaper a,.projectFileArchivePaper .easyui-linkbutton ,.projectFileArchivePaper  input,.projectFileArchivePaper  textarea').addClass('l-btn-disabled');
					}
				}
		 });
	});
...
<div id="projectFileArchiveDiv" class="projectFileArchivePaper">
<div class="easyui-panel archive_dis" title="项目资料归档" style="width:auto">
	<table id="projectFileArchive"></table>
</div>
...
<%--下面DIV是项目归档资料上传查看 --%>
<div align="center" id="projectFileArchiveInfoWindow" class="easyui-window" title="项目归档资料上传查看" data-options="modal:true,closed:true,iconCls:'icon-save',minimizable:false,tools:'#tt'" width="850px" height="400px">
	<form id="projectFileArchiveInfoForm" name="projectFileArchiveInfoForm" method="post">
		...
		<table align="center" cellpadding="5" style="height: auto;">
			<tr>
				<td height="28" align="left"><span class="cus_red">*</span>归档类型：</td>
				<td  colspan="3">
			...
			<tr>
				<td align="left"><span class="cus_red">*</span>备注：</td>
...
<%--下面DIV是利率变更资料上传弹出框 --%>
<div align="center" id="archiveFileInfoWindow" class="easyui-window" title="资料上传" data-options="modal:true,closed:true,iconCls:'icon-save',minimizable:false,tools:'#tt'" width="700px" height="400px">
		...
		<table align="center" cellpadding="5" style="height: auto;">
			...
			<tr>
				...
				<td  colspan="3">
					...
					<input class="easyui-filebox"  name="projectArchiveFile" id="projectArchiveFile" data-options="prompt:'选择一个文件...',onChange:setValueDo,required:true"  style="width:400px">
				</td>
			</tr>
			<tr>
