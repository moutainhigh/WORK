...
var newProjectId="${projectId}";
	...
	<div  class="easyui-panel control_disappr" data-options="collapsible:true" style="padding:10px" title="审批官审批信息" style="width:auto">
		...
		<div id="fkqlstjToolbar" class="easyui-panel" style="border-bottom: 0;">
			<!-- 操作按钮 -->
			<div style="border-bottom: 5px;padding:10px" id="spgbjan">
		...
		<div class="dksqListDiv" style="height:auto;width: auto;display:none;">
			<table id="fkqlstjGrid" disabled="true" title="放款前落实条件列表" class="easyui-datagrid" 
				...
				<thead><tr>
				    ...
				    <th data-options="field:'isConfirm',align:'center',formatter:isConfirmfilter"  >确认</th>
				</tr></thead>
			</table>
		</div>
		<p />
		...
		<div id="dhglyqToolbar" class="easyui-panel" style="border-bottom: 0;">
			...
			<div style="border-bottom: 5px;padding:10px">
				<a href="javascript:void(0)" id="spgbjadd" class="easyui-linkbutton" iconCls="icon-add"  onclick="openItemAddSpxx(2)">新增</a>
				<a href="javascript:void(0)"  id="spgbjedit" class="easyui-linkbutton" iconCls="icon-edit"  onclick="openItemEditSpxx(2)">编辑</a>
				<a href="javascript:void(0)"  id="akeLoanSpg"  class="easyui-linkbutton " iconCls="icon-edit"  onclick="confirmMakeLoan('#dhglyqGrid')">条件确认</a>
				<a href="javascript:void(0)" id="akeLoanCol"  class="easyui-linkbutton " iconCls="icon-edit"  onclick="cancelMakeLoan('#dhglyqGrid')">取消确认</a>				
				<a href="javascript:void(0)" id="spgbjdet"  class="easyui-linkbutton" iconCls="icon-remove" onclick="removeItem('#dhglyqGrid','#dhglyqGrid')">删除</a>
			</div>
		</div>
		...
		<div>
			<form action="<%=basePath%>repaymanageloan/addProcedures.action" id="approveForm" name="approveForm" method="post" >
			...
			<div style="padding-bottom: 20px;padding-top: 10px;">
				<a href="#" class="easyui-linkbutton" id="spgbjesave" style="margin-left: 25%;" iconCls="icon-save" onclick="spgscSave()">保存</a>
			</div>
		</div>
			...
			closed="true" buttons="#dlg_spgsc_but" data-options="modal:true">
	     	<form id="fm_spgsc" method="post">
	     		<div class="fitem" id="update">
	             	...
	             	<input name="infoContent" class="easyui-textbox" style="width:400px;height:40px" required="true" data-options="multiline:true" missingMessage="请录入审批信息!" />
	             	</div>
...
<script type="text/javascript">
	...
	$(function(){
		//debugger;
		// 加载数据的路径
		var urlLS = "<%=basePath%>secondBeforeLoanController/getProjectApprovalLsInfo.action?interestChgId="+interestChgId;
		var urlGL = "<%=basePath%>secondBeforeLoanController/getProjectApprovalGlInfo.action?interestChgId="+interestChgId;
		var urlJl = "<%=basePath%>beforeLoanController/getLoanProjectByProjectId.action?projectId="+newProjectId;
		var urlJl = "<%=basePath%>repaymanageloan/getRepayCgInterestByPid.action?interestChgId="+interestChgId;
		// 加载数据
		...
		if("task_ApprovalOfficerReview"==workflowTaskDefKey || "task_OfflineMeetingResult"==workflowTaskDefKey){
			//审批官审查  线下会议
			if("task_ApprovalOfficerReview"==workflowTaskDefKey){
				approveScrutiny = -1;
			}
		}/* else{
	...
	function saveApprove(){
		var url = "<%=basePath%>secondBeforeLoanController/addProjectApprovalInfo.action?interestChgId="+interestChgId;
		if($("#fm_spgsc input[name='pid']").val() != -1 ){
			url = "<%=basePath%>secondBeforeLoanController/updateProjectApprovalInfo.action?interestChgId="+interestChgId;
		}
	...
	function removeItem(formId,datagridId){
		...
		for(var i=0;i<rows.length;i++){
	 	...
	 	var url = "<%=basePath%>secondBeforeLoanController/deleteProjectApprovalResInfo.action";
	...
	// 新增审批意见
	function spgscSave(){
		$("#approveForm input[name='pId']").val(interestChgId);
		/* var rows = $('#fkqlstjGrid').datagrid('getData');
		for(var i=0;i<rows.total;i++){
			if(rows.rows[i].isConfirm==2){
				$.messager.alert('操作提示', '落实条件确认需要全部确认，才能保存!', 'info');
				return false;
			}
		}
		...
		for(var i=0;i<rows.total;i++){
			if(rows.rows[i].isConfirm==2){
				$.messager.alert('操作提示', '管理要求确认需要全部确认，才能保存!', 'info');
				return false;
			}
		} */
	...
	function confirmMakeLoan(datagridId){
		...
		// 更新路径
		var url = "<%=basePath%>secondBeforeLoanController/updateIsConfirmPrimaryKeyInfo.action";
