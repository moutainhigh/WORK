...
<script type="text/javascript" src="${ctx}/p/xlkfinance/module/repayment/extension_add_investigation.js"></script>
...
var Iam = "EXTENSION";
/**************工作流字段 begin******************/
var WORKFLOW_ID = "extensionRequestProcess"; 
workflowTaskDefKey = "task_ExtensionRequest";
/**************工作流字段 end********************/
	...
	$('#tabs').tabs({
		onSelect:function(title){
	  		...
	  		if(title == "展期申请"){
	  			...
	  			p.panel('refresh','<%=basePath%>loanExtensionController/toLoanExtensionInvestigation.action');
	  		}
	  		// 2.合同
	  		else if(title == "合同"){
	  			if(newProjectId == -1){
	  			...
	  			p.panel('refresh','<%=basePath%>loanExtensionController/navigationContract.action?projectId='+newProjectId+'&contractCatelog=CREDIT_CONTRACT');
	  		}
	  		// 3.借据及还款计划表
	  			...
	  			p.panel('refresh','<%=basePath%>loanExtensionController/receiptRepaymentList.action?loanId='+loanId+'&projectId='+newProjectId+'&contractCatelog=JJ');
	  		}
	  		// 4.资料上传
	  		else if(title == "资料上传"){ 
	  			if(newProjectId == -1){
	  			...
	  			p.panel('refresh','<%=basePath%>loanExtensionController/navigationDatum.action');
	  		} 
		}
	});
...
<div class="easyui-panel" title="基本项目信息" data-options="collapsible:true">
	<div style="padding:5px">
		<table>
			<tr>
				<td height="35" width="300" align="center">项目名称: <a href="#" onclick="formatterProjectName.disposeClick(${projectId},'${projectNum}')"><font color='blue'> ${projectName }</font></a></td>
...
<div class="easyui-tabs" id="tabs" style="width:auto;height:auto">
	...
	<div title="客户信息" id="tab1">
		...
		<div id="enterprise" class="none">
			<form action="<%=basePath%>loanExtensionController/addExtensionProject.action" id="enterpriseForm" name="enterpriseForm" method="post">
		...
		<div id="personal" class="none">
			<form action="<%=basePath%>loanExtensionController/addExtensionProject.action" id="personalForm" name="personalForm" method="post" >
...
<div id="include_approve_scrutiny">
	<%@ include file="procedures/approve_scrutiny.jsp" %>
</div>
<div id="include_organization">
	<%@ include file="procedures/organization_commission.jsp" %>
</div>
<div id="include_offlineCreditMeetingSummary">
	<%@ include file="procedures/offlineCreditMeetingSummary.jsp" %>
</div>
<div id="include_projectArchive">
	<%@ include file="procedures/projectArchive.jsp" %>  
</div>
<div id="include_approve_dizhiya">
...
$(document).ready(function() {
	...
	if("task_ExtensionRequest" != workflowTaskDefKey){
		$("#enterpriseSave").linkbutton("disable");
		$("#personalSave").linkbutton("disable");
	}
