 ...
 <div class="contract_dis">
	<div  style="padding: 30px 10px 10px 10px;width:1039px;">
			<div id="contractList">
				 <div id="toolbar_contract"  class="easyui-panel" >
					...
					<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="editCreditContract('ZK')">编辑</a>
					<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="deleteCreditContract('ZK')">删除</a>
				</div>
				<div class="contractListDiv" id="creditContractDiv">
					<table id="grid_contract" title="授信合同" class="easyui-datagrid" 
						style="height:250px;width: auto;"
						data-options="
						    url: '${basePath}creditsContractController/getContractListUrl.action?projectId='+newProjectId+'&contractCatelogKey=ZK,SX,DK&oldProjectId='+projectId,
						    method: 'POST',
						    collapsible:true,
						    rownumbers: true,
						    singleSelect: false,
	...
	<div id="openContract" class="easyui-dialog" style="width:870px;height:400px;padding:10px;" 
		...
		<div class="fitem">
            ...
            <input name="" id="templateName" class="easyui-combobox" style="width: 320px;" editable="false" data-options="valueField:'pid',textField:'comboboxTemplateText',url:'<%=basePath%>beforeLoanController/getTempLateList.action'" />
...
<script type="text/javascript">
   var assBaseIds =""; // 选择的合同附件
	...
	function openCreditsWindow(type){
		...
		if(type == 1){
			// 把URL地址改变为授信的接口
			url = BASE_PATH+"creditsController/getProjectAcctAndPublicManByProjectId.action?projectId="+newProjectId;
			xzflag=false;
			$('#contractType').val("ZK");
		}else if(type == 2){
			// 把URL地址改变为抵押的接口
			url = BASE_PATH+"mortgageController/getProjectAssBaseByMortgageGuaranteeType.action?projectId="+newProjectId+"&mortgageGuaranteeType=1&oldProject=-1";
			$('#contractType').val("DY");
		}else if(type == 3){
			// 把URL地址改变为质押的接口
			url = BASE_PATH+"mortgageController/getProjectAssBaseByMortgageGuaranteeType.action?projectId="+newProjectId+"&mortgageGuaranteeType=2&oldProject=-1";
			$('#contractType').val("ZY");
		}else if(type == 4){
			// 把URL地址改变为保证的接口
			url = BASE_PATH+"secondBeforeLoanController/getAssureByProjectId.action?projectId="+newProjectId;
			$('#contractType').val("ZKBZ");
		}
		var customerType=0;
		if(type !=1){
	...
	function openCreditContract(){
		...
		if(mcontracType == "ZKBZ"){
			// 赋值担保ID
			$("#suretyId").val(rows[0].pid);
			$("#mortgagorId").val("");
			templateCatelog="ZKBZ,BZ";
		}else if(mcontracType == "DY"){
			// 赋值抵质押物ID
			mortgagorType=1;
		...
		//绑定合同模板-父合同下拉框
		getContractTemplateCombox('templateName','add_parent_contract_id','templateDialog',customerType,templateCatelog,newProjectId);
	...
	function ajaxContract(){
		...
		loadProjectContract("templateDialog",contractType,tempLateId,newProjectId,projectId,newProjectId,mortgagorId,suretyId,projectId,mortgagorType,commonDebtorPids,assBaseIds);
	...
	function saveCreditContract(){		
		...
		if(checkContractCount(newProjectId,tempLateId,3)){
			$.messager.alert("操作提示", "一个展期项目不能新增多个展期借款合同!", "error");
			return;
		}
		//判断是否要选择父合同
		if(isParent==1){
		...
		$.ajax({
	        ...
	        success: function(data){
	        	 $.messager.alert("操作提示", "生成合同成功！", "success");
        		 $(mgrid).datagrid('reload');
	        ...
	        error: function(data){
	        	$.messager.alert("操作提示", "合同生成错误！", "error");
	        }
		}); 
	}
	...
	function saveEidtContent(){
		...
		$.ajax({
	        ...
	        dataType: "json",
	        success: function(data){
	        	$.messager.alert("操作提示", "编辑成功！", "success");
	        	 $(mgrid).datagrid('reload');
	...
	function changeEvent(mcontracType,str){
		mgridType(mcontracType);
		$(mgrid).datagrid({url:"${basePath}creditsContractController/getContractListUrl.action?customerType="+str+"&projectId="+newProjectId+"&contractCatelogKey="+contractCatelog }); 
		$(mgrid).datagrid('clearChecked');
	}
	$(document).ready(function(){
		// 老项目的授信项目不能操作
		applyCommon.creditContract_gridOnLoadSuccess(projectId);
		// 详情链接点击进来,表单禁用
		...
		if("task_ExtensionRequest"==workflowTaskDefKey || "task_ApprovalOfficerReview"==workflowTaskDefKey ||"task_OfflineMeetingResult"==workflowTaskDefKey||"task_ContractSign"==workflowTaskDefKey){
...
<div id="include_projectArchive2">
	<%@ include file="extension_contract_like.jsp" %>  
</div>	
