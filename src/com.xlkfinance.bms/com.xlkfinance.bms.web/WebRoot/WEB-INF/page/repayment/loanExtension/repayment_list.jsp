...
$(function(){
	//$('.creditAmtFormatter').html(accounting.formatMoney(new Number($('.creditAmtFormatter').text()), "", 2, ",", "."));
	...
	<div id="cus_content">
			<table style="border:none;width:700px;">
				<tr>
					...
					<td colspan="2" width="240">${repayment.acctName}</td>
					<td class="label_right">原借款合同编号：</td>
					<td>${olRepayment.contractNo}</td>
					<td class="label_right" style="width:120px;">展期借款合同编号：</td>
					<td colspan="2" width="240">${repayment.contractNo }</td>
				</tr>
				<tr>
					<td class="label_right">放款日期：</td>
					<td colspan="2">${olRepayment.loanOutDt}</td>
					<td class="label_right">展期日期：</td>
					<td>${repayment.loanOutDt}</td>
					<td class="label_right">最后还款日：</td>
					<td colspan="2">${repayment.loanRepayDt }</td>
				</tr>
				<tr>
					<td class="label_right">借款展期期限：</td>
					<td colspan="4">${repayment.repayCycle}</td>
					<td class="label_right">借款金额：</td>
					<td colspan="2" >${repayment.formatAmt }<!-- <div class="creditAmtFormatter">${repayment.creditAmt }</div> --></td>
				</tr>
				<tr>
...
<form id="repaymentPlanForm" method="post">
	...
	<div id="exportDiv" style="border-bottom: 0;">
		<div id="#toolbar_export" class="easyui-panel datagrid-toolbar" style="border-bottom: 0;">
			<a href="javascript:void(0)" class="easyui-linkbutton download"  iconCls="icon-export" plain="true" onclick="exportRepay.exprotRepayment('REPAYMENT_EXTENSION',${loanId },newProjectId,'${isEdit}')">导出Excel</a>
		</div>
	</div>			
