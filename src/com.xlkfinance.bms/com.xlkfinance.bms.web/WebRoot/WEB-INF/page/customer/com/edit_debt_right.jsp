...
function save(){
	...
	$("#baseInfo input[name='warWay']").val(warWay);
	   $('#baseInfo').form('submit', {
			url : "saveComDebt.action",
			onSubmit : function() {return $(this).form('validate');},
			success : function(result) {
				window.location.href="listComDebt.action?comId=${comId}&acctId=${acctId}";
			}
		}); 
}
$(function(){
...
<table  class="cus_table" style="border:none;">
  <tr>
	 ...
	 <td class="align_right" style="width: 150px;"><span class="cus_red">*</span>债务人：</td>
	 ...
	 <td><input type="text" class="text_style easyui-validatebox" name="loanMoney" value="${cusComDebt.loanMoney}" data-options="required:true"/></td>
  </tr>
  <tr>
	 <td class="align_right">债务说明：</td>
	 <td><input type="text" class="text_style" name="debtExpl" value="${cusComDebt.debtExpl}"/></td>
	 <td class="align_right">借款起始日期：</td>
	 <td><input type="text" class="text_style easyui-datebox" editable="false" name="loanStartDate" value="${cusComDebt.loanStartDate}"/></td>
  </tr>
  <tr>
	 ...
	 <td><input type="text" class="text_style" name="repay" value="${cusComDebt.repay}"/></td>
  </tr>
   <tr>
	 ...
	 <td><input type="text" class="text_style easyui-datebox" editable="false" name="lastRepayDate" value="${cusComDebt.lastRepayDate}"/></td>
  </tr>
  <tr>
	 <td class="align_right">偿还方式：</td>
	 <td>
       <select class="select_style easyui-combobox" name="repayWay" editable="false" id="repay_Way">
<!-- 					<option value="1" >一次还请</option> -->
				</select>
     </td>
   ...
   <tr>
	 <td class="align_right">月还款额（万）：</td>
	 <td>
        <input type="text" class="text_style" name="monthPay" value="${cusComDebt.monthPay}"/>
     </td>
