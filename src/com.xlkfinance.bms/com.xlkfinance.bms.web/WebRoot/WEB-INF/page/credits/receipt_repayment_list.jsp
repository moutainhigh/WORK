...
<div class="receipt_dis"  style="padding: 30px 10px 10px 10px;width:1039px">
	<div class="easyui-panel" title="借据" data-options="collapsible:true">		
		...
		<table id="grid_receipt_contract" class="easyui-datagrid" 
			...
			data-options="
			    url: '${basePath}creditsContractController/getContractListUrl.action?projectId='+newProjectId+'&contractCatelogKey=JJ&oldProjectId='+projectId,
			    method: 'POST',
			    rownumbers: true,
			    singleSelect: false,
				border:false,
...
function openCreditsReceiptWindow(){
	...
	var url = BASE_PATH+"creditsController/getProjectAcctAndPublicManByProjectId.action?projectId="+newProjectId;
	
	// 获取datagrid的数据
	$('#receipt_grid').datagrid('options').url = url;
	$('#receipt_grid').datagrid('reload'); 
	$('#receipt_grid').datagrid('clearSelections');
	...
	function openReceiptCreditContract(){
		...
		isReadContract=0;
		getContractTemplateCombox('receiptTemplateName','add_receipt_parent_id','templateReceiptDialog',0,'JJ',newProjectId);
	...
	function ajaxReceiptContract(){
		...
		$.ajax({
	        ...
	        data:{	"contractType":contractType,
	        		...
	        		"debtorProjectId":newProjectId,
	        		"creditProjectId":newProjectId,
	...
	function saveReceiptCreditContract(){
		...
		$.ajax({
	        ...
	        data:{"content":content,"tempLateId":tempLateId,"contractCatelog":contractCatelog,"contractName":contractName,"projectId":newProjectId,"contractParentId":contractParentId},
	        dataType: "json",
	        success: function(data){
	        		 alert("生成借据成功");
	        		 $('#grid_receipt_contract').datagrid('reload');
	...
	$(document).ready(function(){
		// 老项目的借据合同信息是不能修改的
		applyCommon.receiptContract_gridOnLoadSuccess(projectId);
		
 		// 详情链接点击进来,表单禁用
 		applyCommon.detailDisable(biaoZhi,3);
 		
 		if(  workflowTaskDefKey=="task_CreditWithdrawalRequest"|| workflowTaskDefKey=="task_ApprovalOfficerReview"|| workflowTaskDefKey=="task_ContractSign"){
			
 			
		}else{
			$('#tab4 .easyui-linkbutton:not(.download) ,#tab4 input,#tab4 textarea').attr('disabled','disabled');
			$('#tab4 .easyui-linkbutton:not(.download) ,#tab4 input,#tab4 textarea').attr('readOnly','readOnly');
			$('#tab4 .easyui-linkbutton:not(.download) ,#tab4 input,#tab4 textarea').addClass('l-btn-disabled');
		}
	});
</script>
