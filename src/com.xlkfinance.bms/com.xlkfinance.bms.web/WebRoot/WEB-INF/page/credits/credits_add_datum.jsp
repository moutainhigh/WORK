...
<%@ include file="/config.jsp"%>
	<div class="datum" style="padding:10px 10px 0 10px ">
		<div id="toolbar_data">
	...
	$(function(){
		...
		//流程控制
		if( workflowTaskDefKey!="task_ApprovalOfficerAllocation"){
			$('#tab4 .easyui-linkbutton ,#tab4 input,#tab4 textarea').attr('disabled','disabled');
			$('#tab4 .easyui-linkbutton ,#tab4 input,#tab4 textarea').attr('readOnly','readOnly');
			$('#tab4 .easyui-linkbutton ,#tab4 input,#tab4 textarea').addClass('l-btn-disabled');
		}
	});
	...
	function caozuofiter(value,row,index){
		if(row.fileUrl=='' || row.fileUrl==null ||row.fileUrl==undefined){
			...
			var downloadDom = "<a class='download' href='${basePath}beforeLoanController/downloadData.action?path="+row.fileUrl+"&fileName="+row.fileName+"'><font color='blue'>下载</font> | </a>";
		}
		...
		var dataEditDom = "<a href='javascript:void(0)' onclick='editData("+row.dataId+",\""+row.fileDesc+"\",\""+row.filePropertyName+"\","+row.fileProperty+",\""+row.fileName+"\")'><font color='blue'>编辑</font></a> | ";
	...
	function lookfor(){
		...
		if("task_CreditWithdrawalRequest"==workflowTaskDefKey || "task_ApprovalOfficerReview"==workflowTaskDefKey ||"task_OfflineMeetingResult"==workflowTaskDefKey){
