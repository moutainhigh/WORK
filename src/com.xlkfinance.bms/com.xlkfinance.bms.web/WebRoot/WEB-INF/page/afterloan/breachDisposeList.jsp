...
<script type="text/javascript">
	...
	function save() {
		// 是否项目终止
		var isTerm = $("#isTermInAtion").is(':checked');
		if (isTerm) {
			...
			// 由于呆账坏账保存生成记录的时候需要用到页面的数据，所以要先计算一bian
			reCalculationBadDebt();
		}

		$('#myform').form('submit', {
			onSubmit : function() {
				return (isTerm || $(this).form('validate'));
			},
	...
	function saveRepayPlan() {
		...
		if (vpid > 0) {
			//operRepaymentList('${basePath}afterLoanController/advmakeRepayMent.action?projectId=${projectId}&proVioId='+vpid);
			$.ajax({
						type : "POST",
	...
	$(function() {
		...
		if ("${modifyAble}" == "false") {
			setReadOnly();
			projectName = "${bean.projectName}";// 项目名称
		}
		
		// 显示呆账坏账信息
		showBadDetInfo();
		var isTermInAtionVar = '${vio.isTermInAtion}';
		if (isTermInAtionVar != '1') {
			setTimeout(function() {
				$("#badDebtDiv").hide();
			}, 1000);
...
<body class="easyui-layout">
	<div data-options="region:'center'">
		<div id="main_body">
					...
					<div style="">
						<table class="cus_table" style="border: none">
							<tr>
								...
								<td colspan="3"><h2><a  style='color:blue' onclick='formatterProjectName.disposeClick(${bean.projectId},"${bean.projectNumber}")' href='#'>${bean.projectName}</a></h2></td>
							</tr>
							<tr>
			...
			<!-- 违约处理 -->
			<div class="pt10"></div>
			<div class=" easyui-panel" id="titleDiv"  title="违约处理"
				data-options="collapsible:true">
				<form id="myform"
					...
					<div class="dueInfoDiv">
						<table class="cus_table" style="border: none;">
							...
							<tr>
								...
								<input name="isTermInAtion" id="isTermInAtion" type="hidden" value="0" >
							...
							<tr>
								<td colspan="4" align="center" height="35"><input
									type="button" class="save_data text_btn" onclick="save()"
