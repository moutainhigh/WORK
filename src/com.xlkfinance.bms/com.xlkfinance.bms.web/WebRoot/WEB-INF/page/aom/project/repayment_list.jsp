...
$(function(){
	//$('.creditAmtFormatter').html(accounting.formatMoney($('.creditAmtFormatter').text(), "", 2, ",", "."));
	//结束编辑,计算合计
	...
	<div id="cus_content">
			<table style="border:none;width:700px;">
				<tr>
					...
					<td class="label_right" style="width:100px;">借款合同编号：</td>
	...
	<div id="exportDiv" style="border-bottom: 0;">
		<div id="#toolbar_export" class="easyui-panel datagrid-toolbar"  style="border-bottom: 0;">
			<a href="javascript:void(0)" class="easyui-linkbutton download" 
			         iconCls="icon-export" plain="true" onclick="exportRepay.exprotRepayment('REPAYMENT_BEFORELOAN',${loanId },projectId,'${isEdit}')">导出Excel</a>
		</div>			
	</div>	
	<table id="repaymentPlanGrid${isEdit}"  class="easyui-datagrid" 
		...
		<thead>
			<tr>
			    ...
			    <th data-options="field:'total',width:80,formatter:formatMoney,align:'right'${numberEdit }" rowspan="2">合计</th>
			    <th data-options="field:'principalBalance',width:80,formatter:formatMoney,align:'right'${numberEdit }" rowspan="2">本金余额</th>
			</tr>
			<tr>
			    <th data-options="field:'shouldPrincipal',width:100,formatter:formatMoney,align:'right'${numberEdit }">本期应还本金</th>
			    <th data-options="field:'shouldMangCost',width:100,formatter:formatMoney,align:'right'${numberEdit }">本期应付管理费</th>
			    <th data-options="field:'shouldOtherCost',width:90,formatter:formatMoney,align:'right'${numberEdit }">
			    	<c:if test="${isEdit=='1' }">
			    ...
			    <th data-options="field:'shouldInterest',width:100,formatter:formatMoney,align:'right'${numberEdit }">本期应付利息</th>
			</tr>
		</thead>
	</table>
</form>
